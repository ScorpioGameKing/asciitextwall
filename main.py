...
parser: ArgumentParser = ArgumentParser(prog='asciitextwall',description='Simple tool to generate ascii text wallpapers',)
...
# All optional arguments, when not found they will use the provided defaults or handled
...
def get_output_path(filename:str) -> str:
    ...
    # We're either incrementing our saved file or overwriting the existing file
    if args.increment:
        root, ext = splitext(p=filename)
        counter = 1
        while exists(path=output_name):
            output_name: str = f"{expanduser(path=args.output)}{root}_{counter}{ext}"
            counter += 1
    else:
...
def main() -> None:
    ...
    # Create the base image and FigletString
    ...
    # Create a font and convert the FigletString into text to Draw
    # NOTE A monospaced font is needed in this case, because the characters might be missaligned otherwise
